...
class Shopactivity extends StatefulWidget {
  ...
  @override
  ShopactivityState createState() => ShopactivityState();
}

class ShopactivityState extends State<Shopactivity> with ChangeNotifier {
  ...
  getproduct(String userid, String searchkey) async {
    try {
      http.Response response = await http
          .get(Uri.parse("${StringConstants.BASE_URL}get_all_products"));
  ...
  onsuccessfeed(Feedresponse.GetFeedResponse feedResponse) {
    if (feedResponse.data != null) {
      if (feedResponse.data!.length > 0) {
        rlshow = true;
        setState(() {
          feed_page_id = feedResponse.data![feedResponse.data!.length - 1].id!;
  ...
  gettagfilter(String userid, String searchkey, String type) async {
    http.Response response = await http.get(Uri.parse(
        "${StringConstants.BASE_URL}tagfilter?searchkey=$searchkey&type=$type&user_id=$userid"));
    // showLoaderDialog(context);
    var jsonResponse = convert.jsonDecode(response.body);
    if (response.statusCode == 200) {
      data = response.body;
      ...
      if (jsonResponse['status'] == 200) {
        ...
        onsuccesstag(
            Feedresponse.getFeedResponseFromJson(data).data, searchkey);
        var venam = jsonDecode(data!)['data'];
        print(venam);
      } else {
        onsuccesstag(null, "");
  ...
  savepost(String userid, String feedId, int type, int pos) async {
    http.Response response = await http
      ...
      body: Container(
        ...
        child: Container(
          ...
          child: Stack(children: [
            ...
            if (widget.type == 1) product(context),
            if (widget.type == 2) experience(context),
            if (widget.type == 3) feed(context)
          ]),
        ),
      ),
    );
      ...
      child: ListView(children: [
        Container(
            ...
            child: const Text("SHOP",
                style: TextStyle(
                    fontSize: 18, color: Colors.black, fontFamily: "Nunito"))),
        Container(
            alignment: Alignment.centerLeft,
        ...
        Container(
          ...
          child: const Text(
              ...
              style: TextStyle(fontSize: 14, color: ColorConstants.txt)),
        ),
        productdata == null
            ? const Center(
                        ...
                        child: ListBody(
                          children: [
                            Container(
                              ...
                              child: Image.network(
                                jsonDecode(data!)['data'][index]['images'][0],
                                fit: BoxFit.fill,
                              ),
                            ),
                            Container(
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      mainAxisAlignment:
                                          MainAxisAlignment.spaceBetween,
                                      children: [
                                        Text(experdata![index].price.toString(),
                                            style: const TextStyle(
                                                fontSize: 14,
                                                color: Colors.black,
                                                fontFamily: "Nunito")),
            ...
            child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  ...
                  return Card(
                    ...
                    child: ListBody(
                      children: [
                        if (databean![index].mediaType.toString().isNotEmpty)
                          SizedBox(
                            height: 300,
                            child: Stack(
                              children: [
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      GestureDetector(
                                        onTap: () {
                                          if (databean![index].type ==
                                              "Modules") {
                                            getmodule(
                                                userid.toString(),
                                                databean![index].id.toString(),
                                                "2");
                                          } else if (databean![index].type ==
                                              "Collections") {
                                            getmodule(
                                                userid.toString(),
                                                databean![index].id.toString(),
                        ...
                        Container(
                          ...
                          child: ListBody(
                            children: [
                              Container(
                                margin: const EdgeInsets.fromLTRB(0, 10, 0, 10),
                                child: Text(databean![index].title!,
                                    style: const TextStyle(
                                        fontSize: 16,
                                        color: Colors.black,
                                        fontFamily: "Nunito",
                              ...
                              SizedBox(
                                ...
                                child: ListView.builder(
                                    ...
                                    physics: const ClampingScrollPhysics(
                                        parent: BouncingScrollPhysics()),
                                    shrinkWrap: true,
                                    itemCount: databean![index].tags!.length,
                                    itemBuilder:
